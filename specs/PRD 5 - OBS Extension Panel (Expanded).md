...
* **Dynamic Catalog:** The panel queries the Mo.Blend API (GET /templates) to fetch available broadcast-specific .mo.blend templates. These are categorized by broadcast utility (e.g., "Alerts", "Stingers/Transitions", "Lower-Thirds", "BRB Screens").  
* **Visual Thumbnails:** Templates are displayed as a compact visual grid optimized for the narrow vertical layout typical of OBS docks. To ensure instant loading, the panel relies on the lightweight .webp thumbnails generated by the template registry, caching them in the CEF local storage.

### **3.2 The Quick Edit UI**

* **Context-Aware Forms:** Upon selecting a template, the panel fetches its specific JSON manifest (GET /manifest) and generates a minimal input form.  
...
2. **The Render Request:** The panel sends a POST /render/export command to the Mo.Blend API.  
...
4. **WebSocket Injection:** Once the API returns a success payload with the absolute file path, the panel uses the obs-websocket-js library to manipulate the live broadcast:  
...
* **Configuration & Portability:** Connection settings (API Port, OBS WebSocket Password, Target Scene names) are stored securely in the CEF browser dock's localStorage. If a dual-PC streaming setup is used, the Mo.Blend headless engine can run on the gaming PC (to utilize its stronger GPU), while the OBS panel runs on the streaming PC, communicating across the local network via configured IP addresses.
